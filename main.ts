function neop_scroll_zch (snr:number,zch_str:string="A",color:number,abstand:number=0) {
    ...
    let farbe:number[]=[NeoPixelColors.Green,NeoPixelColors.Red]; 
...
// console.log(zeichen_matrix.join() +" " +zahl+  " alt: "+n+" zeile:"+zeile+" "+a_bin_zahl.join()+" "+a_bin_zeilen[zeile].join())                 
            //console.log(a_bin_zeilen[zeile]);
        })
    }    
//console.log(a_bin_zeilen[0].join())
// console.log(1)
...
for (let pos=0;pos<=len;pos++) {
    ...
    a_bin_zeilen.forEach (function(a_dig,z) {
        //a_dig=a_bin_zeilen[0]; z=0;
       
        //a_dig.forEach (function(wert,b) {
        //for (let s=0;s<7;s++) {
        //console.log(a_dig.join());
        //console.log(a_dig.join(","))
        for (let b=0;b<mx;b++) {  
            //console.log(a_dig.slice(pos,pos+8).join())
            let wert=a_dig[b+pos];
            if (wert) {
                //let k=Math.floor(b/8)
...
//   neop_schreibe_zch(0,"AB",NeoPixelColors.Red)  
